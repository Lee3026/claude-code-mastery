...
class BucketListApp {
    ...
    createBucketItemHTML(item) {
        ...
        return `
            <div class="bucket-item bg-white rounded-lg shadow-md flex items-center hover:shadow-lg transition-shadow">
                <!-- 체크박스 -->
                <button
                    class="checkbox-btn flex-shrink-0 w-5 h-5 rounded flex items-center justify-center ${checkboxClass} transition-all"
                    onclick="app.handleToggle('${item.id}')"
                >
                    <span class="text-xs font-bold">${checkIcon}</span>
                </button>
                ...
                <!-- 버튼 그룹 -->
                <div class="button-group flex flex-shrink-0">
                    <button
